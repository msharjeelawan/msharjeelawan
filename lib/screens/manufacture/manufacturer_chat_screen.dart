...
class ManufacturerChatScreen extends StatefulWidget {
  final _state=_ChatState();
  String chatId;
  ManufacturerChatScreen({Key? key,required this.chatId}) : super(key: key);

  @override
  State<ManufacturerChatScreen> createState() => _ChatScreenState();
}

class _ChatScreenState extends State<ManufacturerChatScreen> {

  @override
  void initState() {
    super.initState();
    widget._state.getChat(this,widget.chatId);
  }
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  IconButton(
                      onPressed: (){
                        widget._state.sendMsg(myId!, widget.chatId,this);
                      },
...
class _ChatState{
  ...
  void sendMsg(String sender,String chatId,_ChatScreenState state){
    if(formState.currentState!.validate()){
      ...
      FirebaseDatabase.instance.ref("chats").child(chatId).push()
          .update({"id":sender,"msg":msg,"time":"2121"}).then((value) {
            ...
            // print("chat listt size before"+ChatModel.list.length.toString());
            // ChatModel.list.add(modal);
            // print("chat listt size after"+ChatModel.list.length.toString());
            // state.setState(() {
            //
            // });
      }).catchError((e){
  ...
  void getChat(_ChatScreenState state,String chatId) async {
